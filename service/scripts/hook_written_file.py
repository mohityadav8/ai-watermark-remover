...
def resolve_mode(requested: str | None) -> str:
    ...
    for candidate in (
        ...
        if value in MODES:
            return value
        eprint(f"ai-watermark-remover: unknown hook mode {value!r}; using {DEFAULT_MODE}")
        return DEFAULT_MODE
    return DEFAULT_MODE
...
def run_check(path: Path) -> int:
    ...
    _emit(f"ai-watermark-remover: {path.name} carries provenance marks ({detail})")
    eprint(f"ai-watermark-remover: {path} carries AI/C2PA provenance marks:")
...
def run_clean(path: Path) -> int:
    ...
    try:
        ...
        if not isinstance(result, dict) or proc.returncode not in (0, 1):
            ...
            eprint(f"ai-watermark-remover: {path}: {detail}")
    ...
    _emit(
        f"ai-watermark-remover: cleaned {path.name} in place ({summary})",
        additional_context=(
            f"The ai-watermark-remover hook stripped provenance marks from {path} "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        eprint("ai-watermark-remover: hook payload was not valid JSON")
        return EXIT_HOOK_ERROR
    if not isinstance(payload, dict):
        eprint("ai-watermark-remover: hook payload was not a JSON object")
        return EXIT_HOOK_ERROR
    ...
    try:
        ...
        eprint(f"ai-watermark-remover: hook failed on {path}: {error}")
        return EXIT_HOOK_ERROR
