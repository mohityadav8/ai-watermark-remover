#!/usr/bin/env python3
"""HTTP service exposing the ai-watermark-remover cleaning pipeline.
...
def openapi_spec() -> dict[str, Any]:
    ...
    spec: dict[str, Any] = {
        "openapi": "3.0.3",
        "info": {
            "title": "ai-watermark-remover service",
            "version": VERSION,
...
class Handler(BaseHTTPRequestHandler):
    server_version = f"ai-watermark-remover/{VERSION}"
...
def main() -> int:
    ...
    eprint(f"ai-watermark-remover service {VERSION} on http://{args.host}:{args.port}")
    try:
