```
____ _    _ _ _ ____ ___ ____ ____ _  _ ____ ____ _  _    ____ ____ _  _ ____ _  _ ____ ____
|__| | __ | | | |__|  |  |___ |__/ |\/| |__| |__/ |_/  __ |__/ |___ |\/| |  | |  | |___ |__/
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python3",
            "args": ["/path/to/ai-watermark-remover/service/scripts/hook_written_file.py",
                     "--mode", "check"],
            "timeout": 30
          }
        ]
...
```
/plugin marketplace add mohityadav8/ai-watermark-remover
/plugin install ai-watermark-remover@ai-watermark-remover
```

The skills then load namespaced: `/ai-watermark-remover:remove-ai-marks` and
`/ai-watermark-remover:clean-user-facing-text` (the bare `/remove-ai-marks` also
works when nothing else claims the name). `/plugin marketplace update
ai-watermark-remover` pulls later versions. The same works from the CLI with
...
| --- | --- | --- |
| `ghcr.io/mohityadav8/ai-watermark-remover:<tag>` / `:latest` | Core HTTP service + all cleaners + exiftool / qpdf / c2patool | Yes |
...
| `ai-watermark-remover-ctrlregen:local` | CtrlRegen pixel removal — **never published** (`noai-watermark` ships no LICENSE) | Local build only |
| `ai-watermark-remover-synthid-scorer:local` | reverse-SynthID scorer — **never published** (non-commercial Research License) | Local build only (CLI scorer + optional `wr-synthid-score` HTTP sidecar under the `heavy` profile) |

Build and run the core service:

```bash
make docker-core-build
docker run --rm -p 127.0.0.1:8765:8765 --read-only --tmpfs /tmp ai-watermark-remover
# any CLI stays runnable by overriding the command:
docker run --rm -v "$(pwd):/data" ai-watermark-remover \
...
docker run --rm \
  ...
  --read-only --tmpfs /tmp \
  -v "$(pwd):/data" \
  ai-watermark-remover-synthid-scorer /data/shot.png
```
...
docker run --rm --user "$(id -u):$(id -g)" -v "$(pwd):/data" \
  ai-watermark-remover-markllm detect /data/wm.txt --scheme kgw --json
```
...
docker run --rm --user "$(id -u):$(id -g)" -v "$(pwd):/data" \
  ai-watermark-remover-markdiffusion detect /data/wm.png --scheme tr --json
```
...
repos:
  - repo: https://github.com/mohityadav8/ai-watermark-remover
    rev: main   # or pin to a tag once you cut a release
    hooks:
      - id: ai-watermark-remover-check   # fails the commit if marks are found
      # - id: ai-watermark-remover-clean # opt-in: cleans staged files in place instead
```
